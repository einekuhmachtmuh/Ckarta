	...
	ck_connection_registry_output_pin_t *pin)
{
	...
	if (pin->connection != NULL || pin->writer != NULL || pin->handle != 0)
	{
		return -1;
	}
	...
	if (ck_connection_state(&entry->connection) == CK_CONNECTION_CLOSED
			|| entry->connection.http_writer == NULL)
	{
		(void)pthread_mutex_unlock(&registry->lock);
		return 1;
	}
	...
	entry->output_users++;
	pin->connection = &entry->connection;
	pin->writer = entry->connection.http_writer;
	pin->handle = handle;
	...
	ck_connection_registry_output_pin_t *pin)
{
	...
	if (registry == NULL || !registry->initialized || pin == NULL
			|| pin->connection == NULL || pin->writer == NULL
			|| pin->handle == 0)
	{
		return -1;
	}
	...
	if (!entry->active || entry->generation != generation
			|| &entry->connection != pin->connection
			|| entry->connection.http_writer != pin->writer
			|| entry->output_users == 0)
	{
		(void)pthread_mutex_unlock(&registry->lock);
		return -2;
	...
	uint64_t lifetime_token)
{
	...
	if (entry->reader_users != 0 || entry->output_users != 0)
	{
		(void)pthread_mutex_unlock(&registry->lock);
		return 2;
	}

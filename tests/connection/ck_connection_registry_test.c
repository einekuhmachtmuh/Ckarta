...
int main(void)
{
	...
	ck_connection_registry_output_pin_t output_pin = {0};
	...
	assert(ck_connection_registry_output_acquire(
			&registry, first, 11, 22, 33, &output_pin) == 0);
	assert(output_pin.connection == pin.connection);
	assert(output_pin.writer == output_pin.connection->http_writer);
	...
	assert(ck_connection_registry_output_release(&registry, &output_pin) == 0);
	assert(output_pin.connection == NULL);
	assert(output_pin.writer == NULL);
	assert(output_pin.handle == 0);

...
static void test_content_length_body_and_pipeline(void)
{
	...
	result = ck_http_input_feed(&input, request_and_body,
			header_length + 5 + 7, &consumed, &body, &body_length);
	...
	assert(strcmp(input.request.method.data, "POST") == 0 ||
			input.request.method.length == 4);
	assert(input.request.body_mode == CK_HTTP_BODY_CONTENT_LENGTH);

	ck_http_input_next_request(&input);
	result = ck_http_input_feed(&input,
			request_and_body + header_length + 5,
			strlen(request_and_body) - header_length - 5,
			&consumed, &body, &body_length);
	assert(result == CK_HTTP_INPUT_COMPLETE);
	assert(consumed == strlen("GET /next HTTP/1.1\r\nHost: example.test\r\n\r\n"));
...
static void test_chunked_body_and_pipeline(void)
{
	...
	while (offset < strlen(input))
	{
		...
		if (body_length != 0)
		{
			if (total_body < 4)
			{
				assert(memcmp(body, "Wiki" + total_body,
						body_length <= 4 - total_body ? body_length : 4 - total_body) == 0);
			}
		...
		if (consumed == 0)
		{
			assert(feed_length == 0);
		}
	}

	assert(ck_http_input_complete(&state));
	assert(total_body == 9);
	assert(consumed < 3 || offset == strlen(input));
}

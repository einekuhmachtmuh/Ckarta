...
目前 Linux completion notification smoke path 也已接入 `eventfd → ck_event_loop_wait()`；這是單一 executable integration，不代表正式 multi-worker/cross-platform notification backend。Windows 候選仍為 IOCP + PostQueuedCompletionStatus。

## 11. Java completion notification

目前 Linux 已有單一 executable completion notification path：Java executor → JNI publisher → bounded native completion queue → eventfd → `ck_event_loop_wait()` → notification drain → completion routing。這與一般 listener/event consumer 的責任分離一致，但尚未形成正式 multi-worker、cross-platform completion backend。

Windows 則尚未實作對應 IOCP completion notification；待 socket/event backend contract 收斂後再獨立處理。
...
Linux `ck_event_loop` baseline：已實作；最新 `main` HEAD 的 GitHub Actions 驗證須以當前 run 狀態為準。
Linux loopback TCP listener／accepted connection integration：已實作；最新 `main` HEAD 的 GitHub Actions 驗證須以當前 run 狀態為準。
Windows IOCP backend：尚未實作。
正式 multi-worker network event consumer：尚未實作。
正式 multi-worker/cross-platform completion notification backend：尚未實作。
